...
```{r incedentsByMonthHourly, message=FALSE}
hourly_data %>%
  group_by(Time_Decimal, Month) %>% 
  summarize(Incidents = n(), .groups = "drop") %>%
ggplot(aes(x = Time_Decimal, y = Incidents, color=Month)) +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 23, by = 1), labels = sprintf("%02d:00", 0:23)) +
  labs(title = "Incidents by Month Hourly",
       x = "Hour",
       y = "Number of Incidents") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
